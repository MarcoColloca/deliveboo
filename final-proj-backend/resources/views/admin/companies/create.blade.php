...
<div class="container bg-light mt-5 mb-5 rounded-4 p-5 text-blue shadow">
    ...
    <form action="{{ route('admin.companies.store')}}" method="POST" class="my-company-form" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" required name="name" class="form-control" id="name" placeholder="Inserisci il nome" value="{{ old('name') }}" maxlength="250">
        </div>
        <div class="mb-3">
         <label for="city" class="form-label fb-bold">Città *</label>
         <input type="text" required class="form-control" name="city" id="city" placeholder="Inserisci la città" value="{{ old('city') }}" maxlength="250">
        </div>
        <div class="mb-3">
            <label for="address" class="form-label fb-bold">Indirizzo *</label>
            <input type="text" required class="form-control" name="address" id="address" placeholder="Inserisci la via" value="{{ old('address') }}" maxlength="250">
        </div>
        <div class="mb-3">
            <label for="vat_number" class="form-label fb-bold">P.iva *</label>
            <input type="text" required class="form-control" name="vat_number" id="vat_number" placeholder="Inserisci la p.iva" value="{{ old('vat_number') }}" minlength="11" maxlength="11">
        </div>
        <div class="mb-3">
            <label for="phone_number" class="form-label fb-bold">Telefono *</label>
            <input type="tel" required  class="form-control" name="phone_number" id="phone_number" placeholder="Inserisci il numero di telefono" value="{{ old('phone_number') }}" maxlength="50">
        </div>
        <div class="mb-3">
            <label for="email" class="form-label fb-bold">Email *</label>
            <input type="email" required class="form-control" name="email" id="email" placeholder="Inserisci la tua email" value="{{ old('email') }}" maxlength="255">
        </div>
        <div class="mb-3">

...
<div class="container bg-light mt-5 mb-5 rounded-4 p-5">
    ...
    <form action="{{ route('admin.companies.update', $company)}}" method="POST"  enctype="multipart/form-data">
        ...
        <div class="mb-3">
         ...
         <input type="text" class="form-control" name="city" id="city" placeholder="Inserisci la città" value="{{ old('city', $company->city) }}" >
        </div>
        <div class="mb-3">
            <label for="address" class="form-label fb-bold">Via</label>
            <input type="text" class="form-control" name="address" id="address" placeholder="Inserisci la via" value="{{ old('address', $company->address) }}">
        </div>
        <div class="mb-3">
            <label for="vat_number" class="form-label fb-bold">P.iva</label>
            <input type="text" class="form-control" name="vat_number" id="vat_number" placeholder="Inserisci la p.iva" value="{{ old('vat_number', $company->vat_number) }}">
        </div>
        <div class="mb-3">
            <label for="phone_number" class="form-label fb-bold">Telefono</label>
            <input type="text" class="form-control" name="phone_number" id="phone_number" placeholder="Inserisci il numero di telefono" value="{{ old('phone_number', $company->phone_number) }}">
        </div>
        <div class="mb-3">
            <label for="email" class="form-label fb-bold">Email</label>
            <input type="email" class="form-control" name="email" id="email" placeholder="Inserisci la tua email" value="{{ old('email', $company->email) }}">
        </div>
        <div class="mb-3">
